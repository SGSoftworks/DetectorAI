...
`;
    ...
    try {
      ...
      return {
        isAI: result.toLowerCase().includes('ia') || result.toLowerCase().includes('artificial'),
  ...
  async analyzeWithHuggingFace(text) {
    ...
    try {
      ...
      console.warn('Error en Hugging Face, usando análisis local:', error.message);
      return this.analyzeWithLocalPatterns(text);
    }
  }
  ...
  analyzeWithLocalPatterns(text) {
    ...
    return {
      ...
      indicators: indicators.length > 0 ? indicators : ['Patrones naturales detectados'],
      ...
      complexity: text.length > 500 ? 'Alta' : text.length > 200 ? 'Media' : 'Baja',
      readability: this.calculateReadability(text),
      fallback: true
    };
  }
  ...
  detectAIPatterns(text) {
    ...
    const aiPhrases = [
      'es importante mencionar', 'cabe destacar que', 'es fundamental',
      'en primer lugar', 'en segundo lugar', 'por un lado', 'por otro lado',
      'en conclusión', 'para finalizar', 'es necesario', 'se debe', 'hay que'
    ];
    ...
    if (lowerText.includes('introducción') && lowerText.includes('conclusión')) {
  ...
  detectHumanPatterns(text) {
    ...
    const colloquialExpressions = [
      'bueno', 'pues', 'entonces', 'o sea', 'digamos', 'tipo', 'como que',
      'me parece', 'creo que', 'pienso que', 'siento que', 'opino que'
    ];
    ...
    if (lowerText.includes("'") || lowerText.includes("q ") || lowerText.includes("xq")) {
  ...
  calculateGeminiConfidence(result) {
    ...
    if (lowerResult.includes('claramente') || lowerResult.includes('definitivamente')) {
      confidence += 0.3;
    }
    if (lowerResult.includes('evidencia') || lowerResult.includes('patrones')) {
      confidence += 0.2;
    }
    if (lowerResult.includes('posiblemente') || lowerResult.includes('podría')) {
      confidence -= 0.2;
    }
